...
import { getCollection, updateCollection, getCollectionsForProduct } from "../../../../actions/collections";
...
export default function EditCollection() {
  ...
  const [currentDisplayOrder, setCurrentDisplayOrder] = useState<number>(0);
    ...
    async function fetchCollection() {
      try {
        ...
        if (collection) {
          ...
          // Store display_order separately to preserve it
          setCurrentDisplayOrder(collection.display_order || 0);

          // Haal producten op die in deze collectie zitten
          try {
            const { data, error } = await supabase
            ...
            if (!error && data) {
              setSelectedProducts(data.map((item: any) => item.product_id));
            }
          } catch (err) {
            console.error("Error fetching products in collection:", err);
          }
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Upload hero image als er een nieuwe file is
      ...
      formDataToSubmit.append("display_order", currentDisplayOrder.toString()); // Preserve existing value
      formDataToSubmit.append("product_ids", selectedProducts.join(","));

      await updateCollection(id, formDataToSubmit);
    } catch (error: any) {
      console.error("Error updating collection:", error);
      alert(error.message || "Fout bij bijwerken van collectie");
        ...
        <div className="grid-12">
          <div className="col-12 md:col-8 md:col-start-3">
            <div className="bg-white rounded-lg p-8 border border-gray-200">
              <h1 className="text-3xl font-bold text-black mb-6">Collectie Bewerken</h1>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  >
                    {isSubmitting || uploading ? "Bijwerken..." : "Collectie Bijwerken"}
                  </button>
                </div>
              </form>
            </div>
